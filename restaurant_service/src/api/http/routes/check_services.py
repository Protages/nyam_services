...
from src.core.celery.tasks import get_customer_service_info
...
async def service_working() -> dict:
    return {'message': 'Hello from restaurant service!'}
...
async def check_customer_service_working():
    ...
    get_customer_service_info.delay(url)
    return CheckServiceResponseSchema(
        response='ok', 
        status_code=5000, 
        request_timedelta=None
    )
...
@router.get('/check/auth', response_model=CheckServiceResponseSchema)
async def check_auth_service_working():
    url = f'{settings.AUTH_SERIVICE_URL}api/v1/service_working'
    return await __check_service_working(url)
