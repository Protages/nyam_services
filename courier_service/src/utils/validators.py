...
async def validate_courier_exist(
    id: int, session: AsyncSession
) -> CourierTable | ObjectDoesNotExistException:
    '''
    Check that Courier with passed `id` exist,
    else raise `ObjectDoesNotExistException`.
    '''
    return await validate_exist(id, CourierTable, session, 'courier')
